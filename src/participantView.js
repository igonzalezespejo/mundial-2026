...
export function initParticipantView(participantId) {
    ...
    const hasPendingKnockouts = appData.actual_knockout_bracket.status === 'PENDING';
    ...
    header.innerHTML = `
        ...
        <div style="display: flex; gap: 1rem; margin-top: 1rem; flex-wrap: wrap;">
            <div><strong>Total:</strong> ${rankingEntry.totalPoints}</div>
        ...
        ${hasPendingKnockouts ? '<p style="margin-top:0.5rem; color:var(--warning); font-size:0.85rem;">Eliminatorias pendientes de resultados reales.</p>' : ''}
    `;
